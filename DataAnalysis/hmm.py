...
def prep_data(pathname, filename, bandwidth):
    ...
    if bandwidth > sampling_rate / 2:
        print(
            f"error: required bandwidth {bandwidth} is higher than what's allowed by the original sampling rate {sampling_rate}."
        )
...
def try_fit(data, num_inits=5, num_states=range(1, 10), verbose=False):
    ...
    for n in tqdm(num_states):
        for i in tqdm(range(num_inits)):
            vi = vhmm.VariationalGaussianHMM(
                n,
                n_iter=2000,
    ...
    return {
        "EM_models": best_models["EM"],
        "VI_models": best_models["VI"],
        "EM_scores": best_scores["EM"],
        "VI_scores": best_scores["VI"],
    }
